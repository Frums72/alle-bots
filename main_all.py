...
import os, requests, re, time, threading, random
...
# ============================================================
BOT_VERSION      = "v60.7"
BOT_VERSION_INFO = "VIP-Werbung jetzt zweisprachig (Deutsch + Englisch in einer Nachricht)"
...
def _af_fixture_zu_intern(m: dict) -> dict:
    ...
    return {
        ...
        "status": _ls_status(m),
        "time": zeit,
        "home": {"id":home_id,"name":home.get("name","?"),"logo":home.get("logo","")},
        "away": {"id":str(away.get("id","")),"name":away.get("name","?"),"logo":away.get("logo","")},
...
def send_telegram(message: str):
    ...
    if resp.status_code != 200:
        ...
        _live_signal_an_gruppe_weiterleiten(message)  # v60.5 NEU

# v60.5 NEU: Spiegelt Live-Signale ab Konfidenz 9/10 zusätzlich (menschlich umformuliert)
# in die Telegram-Gruppe, damit dort auch mal Live-Tipps auftauchen, nicht nur Pre-Match.
# Liest die schon fertig gebaute Nachricht der einzelnen Live-Bots aus (Konfidenz/Liga/
# Team/Tipp) statt jeden der ~15 Live-Bots einzeln umzubauen – rein additiv: wenn das
# Format eines Bots mal nicht exakt passt, wird einfach nichts weitergeleitet (kein
# Fehler, keine Auswirkung auf den eigentlichen Signal-Versand).
_LIVE_SIGNAL_MUSTER = re.compile(
    r"Konfidenz:\s*<b>(\d+)/10</b>.*?"
    r"🏆\s*([^\n(]+?)\s*(?:\([^)]*\))?\s*\n"
    r"(?:📌|⚽)\s*([^\n<]+?)\s+vs\s+([^\n<]+?)\s*\n",
    re.DOTALL,
)
_LIVE_TIPP_MUSTER = re.compile(r"🎯 Tipp:\s*([^\n]+)")
LIVE_WEITERLEITUNG_MIN_KONFIDENZ = 9

def _live_signal_an_gruppe_weiterleiten(text: str):
    try:
        m = _LIVE_SIGNAL_MUSTER.search(text)
        if not m:
            return
        konfidenz = int(m.group(1))
        if konfidenz < LIVE_WEITERLEITUNG_MIN_KONFIDENZ:
            return
        home = (m.group(3) or "").strip()
        away = (m.group(4) or "").strip()
        tm   = _LIVE_TIPP_MUSTER.search(text)
        tipp = re.sub(r"<[^>]+>","",tm.group(1)).strip() if tm else ""
        if not (home and away and tipp):
            return
        intros = [
            f"👀 Schau grad auf {home} vs {away} –",
            f"Bei {home} - {away} tut sich gerade was:",
            f"{home} gegen {away} live gerade richtig interessant:",
            f"Live gerade bei {home} - {away}:",
        ]
        konf_saetze = ["bin mir da echt sicher", "sehr hohe Überzeugung gerade",
                        "sieht richtig gut aus dafür", "läuft grad genau so wie erhofft"]
        gruppen_msg = f"{random.choice(intros)} {tipp} ({random.choice(konf_saetze)}, {konfidenz}/10) ⚡"
        send_telegram_gruppe(gruppen_msg)
    except Exception as e:
        print(f"  [Live-Weiterleitung] Übersprungen ({e})")
...
def send_telegram_gruppe_mit_bild(caption: str, home_logo: str = "", away_logo: str = "", chat_id: str = None):
    """v60.4 NEU: Schickt eine Nachricht mit den beiden Team-Logos (falls livescore-api.com
    welche mitliefert) als kleine Bilder-Galerie mit Text darunter. Fällt automatisch auf
    eine normale Text-Nachricht zurück, wenn keine Logos vorhanden sind oder der
    Bild-Versand aus irgendeinem Grund fehlschlägt (z.B. tote Bild-URL)."""
    ziel = chat_id or TELEGRAM_CHAT_PREMATCH
    logos = [l for l in (home_logo,away_logo) if l]
    if not logos:
        send_telegram_gruppe(caption, chat_id=ziel)
        return
    try:
        media = [{"type":"photo","media":l} for l in logos]
        media[-1]["caption"]    = caption
        media[-1]["parse_mode"] = "HTML"
        url  = f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/sendMediaGroup"
        resp = requests.post(url,json={"chat_id":ziel,"media":media},timeout=15)
        if resp.status_code != 200:
            print(f"  [Telegram] Bild-Versand fehlgeschlagen ({resp.status_code}) – Fallback auf Text")
            send_telegram_gruppe(caption, chat_id=ziel)
    except Exception as e:
        print(f"  [Telegram] Bild-Versand Fehler: {e} – Fallback auf Text")
        send_telegram_gruppe(caption, chat_id=ziel)
...
# v60.3 BUGFIX: Die echte livescore-api.com Antwort sieht laut offizieller Doku so aus:
# {"success":true,"data":{"corners":"4:13","shots_on_target":"5:3","saves":"2:2",
#  "possesion":"48:52", ...}} – also ein DICT mit "Heim:Auswärts"-Strings, KEINE Liste
# von {"type":...,"home":...,"away":...} Objekten wie beim ersten Umbau angenommen.
# Das war der Grund, warum Ecken/Schüsse/Paraden/Ballbesitz seit der Umstellung immer
# als 0 zurückkamen (der alte Code ist bei jedem Aufruf auf einen stillen Fehler
# gelaufen) – und damit alle Bots, die auf diesen Werten basieren (Ecken-Bot,
# Torwart-Bot, Druck-Bot, Comeback-Bot, ...), praktisch nie ausgelöst haben.
_LS_STAT_FELD_MAP = {
    "corners":           ("corners_home","corners_away"),
    "shots_on_target":   ("shots_on_target_home","shots_on_target_away"),
    "saves":             ("saves_home","saves_away"),
    "possesion":         ("possession_home","possession_away"),  # Tippfehler kommt so von livescore-api.com
}

def _ls_stat_teile(wert: str):
    """Wandelt einen '4:13'-String in (4,13) um. Manche kleinen Ligen liefern hier
    laut Doku auch mal None/null statt einem String – dann (0,0)."""
    if not wert or not isinstance(wert,str) or ":" not in wert:
        return 0,0
    try:
        h,a = wert.split(":",1)
...
def ls_get_statistiken(match_id):
    ...
    try:
        params = {"id":match_id}  # v60.3 BUGFIX: Parameter heißt "id", nicht "match_id" (siehe Doku)
        resp   = api_get_with_retry(f"{LS_BASE}/matches/statistics.json",params)
        data   = resp.json().get("data") or {}
        for api_feld,(feld_home,feld_away) in _LS_STAT_FELD_MAP.items():
            h,a = _ls_stat_teile(data.get(api_feld))
            if "possession" in feld_home:
                result[feld_home], result[feld_away] = str(h), str(a)
            else:
                result[feld_home], result[feld_away] = h, a
    except Exception as e:
...
def send_vip_werbung():
    """v60.5 FIX: Geht jetzt an die echte Telegram-Gruppe (send_telegram_gruppe), nicht
    mehr privat an den Admin-Chat. Vorher landete die Nachricht bei TELEGRAM_CHAT_ID
    (privater Admin-Kanal) und musste manuell in die Gruppe weitergeleitet werden.
    v60.7 NEU: Jetzt zweisprachig (Deutsch + Englisch) in einer Nachricht."""
    ...
    msg_en = (
        ...
        "━━━━━━━━━━━━━━━━━━━━\n\n"
        "Most people in this group only see a fraction of what's really possible "
        "here. The free tips are just the start – a small taste of what our "
        "<b>VIP members</b> get every single day.\n\n"
        "🏆 <b>As a VIP member you get:</b>\n"
        "✅ Exclusive tips with much higher confidence\n"
        "✅ More signals – earlier, more often & straight from live matches\n"
        "✅ Full AI analysis on every single tip\n"
        "✅ Personal support & direct access to me\n"
        "✅ A small, tight-knit community instead of an anonymous crowd\n\n"
        "━━━━━━━━━━━━━━━━━━━━\n"
        "💭 <i>Imagine you'd had the VIP tips all week already...</i>\n\n"
        "Spots in the VIP group are deliberately limited – not everyone who asks "
        "gets in. That's how we keep the quality high and make sure every member "
        "gets the attention they deserve.\n\n"
        "A lot of people wait too long and end up regretting the tips they "
        "missed. Don't wait for the perfect moment – it never really comes.\n\n"
        "━━━━━━━━━━━━━━━━━━━━\n"
        f"📩 <b>Interested?</b> Just message me privately on Telegram: <b>@{VIP_TELEGRAM_HANDLE}</b>\n"
        "Happy to answer any questions and figure out together if VIP is right "
        "for you.\n"
        "━━━━━━━━━━━━━━━━━━━━\n\n"
    ...
    print("  [VIP-Werbung] ✅ Werbenachricht in die Gruppe gesendet")
...
def _menschliche_prematch_nachricht(analysen: list, now) -> str:
    """v60.4 NEU: Baut die Pre-Match-Nachricht als fließenden, locker formulierten Text
    statt starrem 🎯/🟡-Raster – wirkt dadurch wie handgeschrieben statt automatisiert.
    Formulierungen werden per Zufall variiert, damit sich nicht jeder Post gleich liest."""
    tageszeit = "heute Vormittag" if now.hour == 10 else ("heute Nachmittag" if now.hour == 16 else "heute Abend")
    wrapper_intros = [
        f"Hab mir grad ein paar Spiele für {tageszeit} angeschaut, hier meine Tipps 👇",
        f"So, hier sind meine Tipps für {tageszeit} 👇",
        f"Ein paar Spiele hab ich mir für {tageszeit} rausgesucht:",
        f"Kurzer Blick auf {tageszeit} – das sind meine Favoriten:",
    ]
    tipp_intros = [
        "Bei {home} gegen {away}",
        "Für {home} vs {away}",
        "Schau dir mal {home} - {away} an,",
        "{home} gegen {away} –",
    ]
    konf_hoch  = ["da bin ich mir ziemlich sicher", "einer meiner Favoriten heute", "da hab ich ein gutes Gefühl"]
    konf_mit   = ["sieht für mich solide aus", "würde ich als soliden Tipp einstufen", "sehe da gute Chancen"]
    konf_nied  = ["ist eher etwas spekulativer", "würde ich mit Vorsicht genießen", "ist ein kleinerer Außenseiter-Tipp"]
    schluss_varianten = [
        "Wie immer: kein Wett-Zwang, einfach meine ehrliche Einschätzung 🤝",
        "Wie gewohnt gilt: eigene Recherche nie ganz sein lassen 👀",
        "Viel Erfolg, falls ihr mitgeht!",
    ]

    teile = [random.choice(wrapper_intros), ""]
    for a in analysen:
        konf = a["konfidenz"]
        konf_phrase = random.choice(konf_hoch) if konf >= 8 else (random.choice(konf_mit) if konf >= 6 else random.choice(konf_nied))
        intro = random.choice(tipp_intros).format(home=a["home"],away=a["away"])
        liga_str = a["liga"]+(f" ({a['country']})" if a["country"] else "")
        teile.append(
            f"⚽ <b>{intro}</b> ({liga_str}, Anstoß {a['anstoß']} Uhr)\n"
            f"Mein Tipp: <b>{a['tipp']}</b> – {konf_phrase} ({konf}/10).\n"
            f"{a['analyse']}\n"
        )
    teile.append(f"💬 Fragen/Feedback? Meldet euch gerne: discord.gg/G6dt3Kpf")
    teile.append(random.choice(schluss_varianten))
    teile.append("⚠️ 18+ | Verantwortungsvoll spielen")
    return "\n".join(teile)
        ...
        try:
            ...
            if now.hour in PREMATCH_UHRZEITEN and now.minute < 5 and key not in gesendet:
                ...
                for spiel in auswahl:
                    ...
                    time.sleep(1)
                if analysen:
                    msg = _menschliche_prematch_nachricht(analysen, now)  # v60.4: menschlicher formuliert
                    send_telegram_gruppe(msg)
...
_social_proof_letzter_stand = None

def bot_social_proof():
    """v60.5 NEU: Postet ab und zu eine lockere 'Trefferquote'-Nachricht in die Gruppe
    ("Jo Jungs, X/Y Tipps gehittet"), damit die Gruppe lebendiger wirkt. Baut IMMER auf
    echten Werten aus dem Pre-Match-Tracker auf, nie erfundene Zahlen. Prüft alle 30
    Minuten, postet aber nur wenn sich seit dem letzten Post was geändert hat UND die
    Quote der letzten 3-4 ausgewerteten Tipps gut aussieht (>=50% Trefferquote) – bei
    einer schlechten Phase bleibt der Bot einfach still, statt Verlust-Serien zu feiern."""
    global _social_proof_letzter_stand
    print("[Social-Proof-Bot] Gestartet | alle 30 Min Check")
    while True:
        try:
            with _prematch_tracker_lock:
                ausgewertet = [v for v in _prematch_tracker.values() if v.get("status") in ("gewonnen","verloren")]
            ausgewertet.sort(key=lambda v: v.get("registriert",""))
            letzte = ausgewertet[-4:]
            if len(letzte) >= 3:
                stand = tuple(v.get("status") for v in letzte)
                if stand != _social_proof_letzter_stand:
                    treffer = sum(1 for v in letzte if v.get("status") == "gewonnen")
                    gesamt  = len(letzte)
                    if treffer/gesamt >= 0.5:
                        satz = random.choice([
                            f"Jo Jungs, {treffer}/{gesamt} von unseren letzten Tipps haben gehittet 🔥",
                            f"{treffer} von den letzten {gesamt} Tipps waren Treffer, läuft gerade richtig gut 💪",
                            f"Kleines Update: {treffer}/{gesamt} der letzten Tipps richtig gelegen 👌",
                            f"Läuft grad: {treffer}/{gesamt} Treffer bei den letzten Tipps 🎯",
                        ])
                        send_telegram_gruppe(satz)
                        print(f"  [Social-Proof-Bot] Gepostet: {treffer}/{gesamt}")
                    _social_proof_letzter_stand = stand
        except Exception as e:
            print(f"  [Social-Proof-Bot] Fehler: {e}")
        time.sleep(1800)
        ...
        try:
            ...
            for fixture_id, tipp_info in offene.items():
                ...
                if treffer is None:
                    ...
                    prematch_tracker_speichern()
                    continue
                # v60.5: Auf Wunsch werden verlorene Tipps nur noch STILL im Tracker
                # vermerkt (für die eigene Statistik/Social-Proof-Bot), aber NICHT mehr
                # öffentlich in der Gruppe gepostet – nur noch Gewinne werden gefeiert.
                tipp_text = tipp_info.get("tipp","")
                if not treffer:
                    with _prematch_tracker_lock:
                        _prematch_tracker[fixture_id]["status"]  = "verloren"
                        _prematch_tracker[fixture_id]["endstand"] = score
                    prematch_tracker_speichern()
                    print(f"  [PreMatch-Auswertung] {home} vs {away}: VERLOREN ({score}) – nicht gepostet")
                    continue
                # v60.4: menschlicher formuliert statt starrem "GEWONNEN"-Etikett,
                # Formulierung wird per Zufall variiert.
                gewonnen_phrasen = [
                    "Und tatsächlich – {tipp} ist aufgegangen! 🎉",
                    "Passt! {tipp} hat geklappt.",
                    "Treffer, {tipp} war goldrichtig.",
                    "Nice, {tipp} ist eingetroffen 💪",
                ]
                satz = random.choice(gewonnen_phrasen).format(tipp=tipp_text)
                msg = (f"✅ {satz}\n"
                       f"⚽ {home} {score} {away}\n"
                       f"🕐 {jetzt()} Uhr")
                home_logo = (match.get("home") or {}).get("logo","")
                away_logo = (match.get("away") or {}).get("logo","")
                send_telegram_gruppe_mit_bild(msg, home_logo, away_logo)  # v60.4: mit Team-Logos, falls vorhanden
                with _prematch_tracker_lock:
                    _prematch_tracker[fixture_id]["status"] = "gewonnen"
                    _prematch_tracker[fixture_id]["endstand"] = score
                prematch_tracker_speichern()
                print(f"  [PreMatch-Auswertung] {home} vs {away}: GEWONNEN ({score}) – gepostet")
        except Exception as e:
...
# ============================================================
#  v60.6 NEU: INSTAGRAM AUTO-POSTING (Tipp des Tages als Grafik)
# ============================================================
INSTAGRAM_BUSINESS_ID  = os.environ.get("INSTAGRAM_BUSINESS_ID","")
INSTAGRAM_ACCESS_TOKEN = os.environ.get("INSTAGRAM_ACCESS_TOKEN","")
# Railway setzt RAILWAY_PUBLIC_DOMAIN automatisch für Services mit öffentlicher URL
# (z.B. "alle-bots-production.up.railway.app", OHNE https://). Falls das aus
# irgendeinem Grund fehlt, kann INSTAGRAM_HOST_URL manuell gesetzt werden
# (komplett, MIT https://).
_railway_domain = os.environ.get("RAILWAY_PUBLIC_DOMAIN","")
INSTAGRAM_HOST_URL = os.environ.get("INSTAGRAM_HOST_URL", f"https://{_railway_domain}" if _railway_domain else "")

_IG_FONT_PFAD = "/tmp/BettingLab-Font-Bold.ttf"
_IG_FONT_URL  = "https://raw.githubusercontent.com/senotrusov/dejavu-fonts-ttf/master/ttf/DejaVuSans-Bold.ttf"
_letztes_ig_bild = None  # hält die zuletzt erstellte Grafik (Bytes) im Speicher, wird vom Web-Server ausgeliefert

def _ig_font(size: int):
    """Lädt eine echte TTF-Schrift (kann Umlaute/ß/€ etc.), lädt sie beim ersten
    Aufruf einmalig herunter und cached sie in /tmp. Fällt auf Pillows eingebaute
    Mini-Schrift zurück (keine Umlaute, aber kein Absturz), falls der Download aus
    irgendeinem Grund fehlschlägt."""
    from PIL import ImageFont
    if not os.path.exists(_IG_FONT_PFAD):
        try:
            import urllib.request
            urllib.request.urlretrieve(_IG_FONT_URL, _IG_FONT_PFAD)
        except Exception as e:
            print(f"  [Instagram] Font-Download fehlgeschlagen ({e}) – Fallback-Schrift ohne Umlaute")
    try:
        return ImageFont.truetype(_IG_FONT_PFAD, size)
    except Exception:
        return ImageFont.load_default(size=size)

def erstelle_tagestipp_bild(bester: dict) -> bytes:
    """v60.6 NEU: Baut eine 1080x1080 Instagram-Grafik zum 'Tipp des Tages' (Team, Liga,
    Tipp, Konfidenz) im BettingLab-Look (dunkles Navy + Gold, wie das Dashboard)."""
    ...
    def center_text(y, text, size, fill, max_w=920):
        f = _ig_font(size)
        bbox = draw.textbbox((0,0), text, font=f)
        w = bbox[2]-bbox[0]
        while w > max_w and size > 20:  # zu lange Team-/Liganamen automatisch verkleinern
            size -= 4
            f = _ig_font(size)
            bbox = draw.textbbox((0,0), text, font=f)
            w = bbox[2]-bbox[0]
    ...
    draw.rectangle([0,0,W,10], fill=akzent)
    center_text(70,  "TIPP DES TAGES", 32, akzent)
    center_text(150, bester.get("liga","?"), 32, grau)
    center_text(330, bester.get("home","?"), 56, weiss)
    center_text(410, "vs", 32, grau)
    center_text(460, bester.get("away","?"), 56, weiss)
    draw.line([(140,600),(W-140,600)], fill=(48,54,61), width=2)
    center_text(650, bester.get("tipp","?"), 44, akzent)
    center_text(780, f"{bester.get('konfidenz','?')}/10", 88, akzent)
    center_text(890, "KONFIDENZ", 32, grau)
    center_text(1010, "BettingLab", 26, grau)
    draw.rectangle([0,H-10,W,H], fill=akzent)

    buf = io.BytesIO()
    img.save(buf, format="JPEG", quality=92)
    return buf.getvalue()

def instagram_post_tagestipp(bester: dict, caption: str) -> bool:
    """v60.6 NEU: Erstellt die Grafik, hostet sie über den eigenen Railway-Webserver
    (siehe bot_web_dashboard weiter unten) und postet sie über die Instagram Graph API.
    Gibt True/False zurück, ob es geklappt hat – bricht den restlichen Bot NIE ab."""
    global _letztes_ig_bild
    if not (INSTAGRAM_BUSINESS_ID and INSTAGRAM_ACCESS_TOKEN and INSTAGRAM_HOST_URL):
        print("  [Instagram] Übersprungen – INSTAGRAM_BUSINESS_ID/INSTAGRAM_ACCESS_TOKEN/Host-URL nicht vollständig gesetzt")
        return False
    try:
        _letztes_ig_bild = erstelle_tagestipp_bild(bester)
    except Exception as e:
        print(f"  [Instagram] Grafik-Erstellung fehlgeschlagen: {e}")
        return False
    bild_url = f"{INSTAGRAM_HOST_URL.rstrip('/')}/instagram/tagestipp.jpg"
    try:
        r1 = requests.post(
            f"https://graph.facebook.com/v21.0/{INSTAGRAM_BUSINESS_ID}/media",
            data={"image_url":bild_url,"caption":caption,"access_token":INSTAGRAM_ACCESS_TOKEN},
            timeout=20)
        creation_id = r1.json().get("id")
        if not creation_id:
            print(f"  [Instagram] Container-Fehler: {r1.text}")
            return False
        time.sleep(5)  # Instagram braucht kurz, um das Bild von unserer URL zu laden/verarbeiten
        r2 = requests.post(
        ...
        if r2.status_code == 200:
            print("  [Instagram] ✅ Gepostet")
            return True
        print(f"  [Instagram] Publish-Fehler: {r2.text}")
        return False
    except Exception as e:
...
def bot_web_dashboard():
    try:
        ...
        class DashboardHandler(BaseHTTPRequestHandler):
            ...
            def do_GET(self):
                if self.path == "/instagram/tagestipp.jpg":
                    if _letztes_ig_bild:
                        self.send_response(200)
                        self.send_header("Content-Type","image/jpeg")
                        self.send_header("Access-Control-Allow-Origin","*")
                        self.end_headers()
                        self.wfile.write(_letztes_ig_bild)
                    else:
    ...
    while True:
        try:
            ...
            if now.hour==9 and now.minute<5 and key not in gesendet:
                ...
                # v60.6 NEU: gleicher Tipp geht (falls Zugangsdaten gesetzt sind) auch als
                # Grafik automatisch auf Instagram raus.
                ig_caption = (f"⭐ Tipp des Tages – {bester['home']} vs {bester['away']}\n"
                              f"{bester['tipp']} | Konfidenz {bester['konfidenz']}/10\n"
                              f"#sportwetten #fussball #wetttipps")
                instagram_post_tagestipp(bester, ig_caption)
...
if __name__ == "__main__":
    ...
    optional_vars = {
        ...
        "INSTAGRAM_BUSINESS_ID":    INSTAGRAM_BUSINESS_ID,      # v60.6 NEU
        "INSTAGRAM_ACCESS_TOKEN":   INSTAGRAM_ACCESS_TOKEN,     # v60.6 NEU
        "INSTAGRAM_HOST_URL (auto von Railway)": INSTAGRAM_HOST_URL,  # v60.6 NEU
    }
    ...
    bot_definitionen = [
        ...
        ("Social-Proof-Bot", bot_social_proof),  # v60.5 NEU
        ...
        # v60.5: Morgen-Bot komplett entfernt (auf Wunsch) – schickte nur eine reine
        # Spiel-Liste ohne Tipps/Analyse, wirkte wie unnötiger Spam.
