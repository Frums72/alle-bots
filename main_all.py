...
def ls_get_single_match(match_id):
    """Holt ein einzelnes Spiel per ID. Läuft es noch/ist es frisch beendet, steckt es
    im live.json Feed (der bleibt bis zu 3h nach Spielende gefüllt). Ältere, bereits
    abgeschlossene Spiele fallen aus dem Live-Feed raus, dafür gibt es den history.json
    Fallback (gleiche match_id wie im Live-Feed, siehe livescore-api.com Doku)."""
    try:
        params = {"fixture_id":match_id}
        ...
        print(f"  [API] Single-Match (live) Fehler ({match_id}): {e}")
...
if __name__ == "__main__":
    ...
    bot_definitionen = [
        ...
        # v60: Deaktiviert – livescore-api.com liefert keine Über/Unter- oder
        # Mehr-Buchmacher-Quoten mehr, ohne die Value-Bot und Arbitrage-Bot
        # nichts finden können. Code bleibt erhalten, falls ihr später eine
        # zusätzliche Odds-Quelle anbindet.
