...
# ============================================================
BOT_VERSION      = "v59.24"
BOT_VERSION_INFO = "Keine veralteten Vorspiel-Quoten mehr bei Live-Signalen ohne echte Live-Odds – zeigt jetzt ehrlich 'Keine zuverlässige Quote'"
...
def af_get_odds(fixture_id) -> list:
    ...
    _af_odds_cache[key] = {"data": bookmakers, "ts": now, "quelle": quelle}
    return bookmakers

def af_odds_ist_live(fixture_id) -> bool:
    """
    v59.24 NEU: Prüft ob für eine Fixture eine ECHTE Live-Quote vorliegt (nicht nur die
    veraltete Pre-Match-Quote). Wird von den Quoten-Funktionen genutzt, um bei laufenden
    Spielen ohne Live-Odds lieber gar keine Quote zu zeigen statt einer irreführenden
    Vorspiel-Zahl (z.B. "1.05 für Über 0.5 Tore" bei 0:0 in der 62. Minute – das war die
    Vorspiel-Quote für das GESAMTE Spiel, praktisch immer sehr niedrig, aber bei Minute 62
    im 0:0 völlig unpassend).
    """
    key = str(fixture_id)
    if key not in _af_odds_cache:
        af_get_odds(fixture_id)  # Cache befüllen
    cached = _af_odds_cache.get(key)
    return bool(cached and cached.get("quelle") == "live")
...
# v59.18 FIX: Absurde Quoten (21, 42, ...) kamen daher, dass hier bisher einfach die
# HÖCHSTE Quote aus dem GESAMTEN "Goals Over/Under"-Markt genommen wurde – der aber viele
# Linien enthält (Über 0.5 bis Über 5.5, Unter 0.5 usw.). Die höchste Zahl dort ist praktisch
# immer eine absurd unwahrscheinliche Linie (z.B. "Unter 0.5 Tore"), nie die zum Tipp
# passende. Jetzt wird gezielt nach der zur Empfehlung passenden Linie gefiltert; ohne
# passende Linie kommt keine Quote zurück statt einer zufälligen. Zusätzlich ein
# Plausibilitäts-Deckel (max. 12.0), analog zum bereits bestehenden Value-Bot-Sicherheitsnetz.
AF_ODDS_MAX_PLAUSIBLE = 12.0

def _af_odds_linie_filter(werte: list, linie: float = None, richtung: str = None) -> list:
    """Filtert Odds-Werte auf eine konkrete Linie+Richtung (z.B. 'über 2.5'). Ohne Angabe: alles."""
    if linie is None:
        return werte
    ziel_wort = None
    if richtung in ("über","ueber","over"):
        ziel_wort = "Over"
    elif richtung in ("unter","under"):
        ziel_wort = "Under"
    treffer = []
    linie_str = f"{linie}"
    for w in werte:
        if linie_str not in w["label"]:
            continue
        if ziel_wort and ziel_wort not in w["label"]:
            continue
        treffer.append(w)
    return treffer
...
def af_odds_beste_quote(fixture_id, linie: float = None, richtung: str = None, nur_live: bool = True) -> float:
    """
    Quote für eine KONKRETE Linie (z.B. Über 2.5) – kein blindes Maximum über alle Linien mehr.
    v59.24 FIX: nur_live=True (Standard) verweigert die Quote, wenn nur eine veraltete
    Pre-Match-Quote vorliegt (z.B. "Über 0.5 Tore" ist vor Anpfiff fast immer ~1.05, aber bei
    Minute 62 im 0:0 völlig unpassend). Für echte Pre-Match-Anwendungsfälle (TippDesTages-Bot
    etc.) explizit nur_live=False übergeben.
    """
    if nur_live and not af_odds_ist_live(fixture_id):
        print(f"  [AF-Odds] Keine LIVE-Quote für Fixture {fixture_id} – Vorspiel-Quote wird nicht angezeigt")
        return None
    werte = _af_odds_werte(af_get_odds(fixture_id), ("Goals Over/Under","Over/Under"))
    werte = _af_odds_linie_filter(werte, linie, richtung)
    if not werte:
        print(f"  [AF-Odds] Keine Quote für Fixture {fixture_id} (Linie {linie} {richtung or ''})")
        return None
    beste = round(max(w["odd"] for w in werte),2)
    if beste > AF_ODDS_MAX_PLAUSIBLE:
        print(f"  [AF-Odds] ⚠️ Unplausible Quote {beste} für Fixture {fixture_id} verworfen")
        return None
    return beste

def af_odds_details(fixture_id, linie: float = None, richtung: str = None, nur_live: bool = True) -> dict:
    """Details für eine KONKRETE Linie – kein blindes Maximum über alle Linien mehr.
    v59.24 FIX: siehe af_odds_beste_quote – standardmäßig keine Vorspiel-Quote bei laufenden Signalen."""
    if nur_live and not af_odds_ist_live(fixture_id):
        print(f"  [AF-Odds] Keine LIVE-Quote für Fixture {fixture_id} – Vorspiel-Quote wird nicht angezeigt")
    ...
    if not werte:
        print(f"  [AF-Odds] Keine Quote für Fixture {fixture_id} (Linie {linie} {richtung or ''})")
        return {"quote":None,"avg_quote":None,"bookmaker_anzahl":0}
    quotes = [w["odd"] for w in werte if w["odd"] <= AF_ODDS_MAX_PLAUSIBLE]
    if not quotes:
        print(f"  [AF-Odds] ⚠️ Nur unplausible Quoten für Fixture {fixture_id} – verworfen")
        return {"quote":None,"avg_quote":None,"bookmaker_anzahl":0}
...
def get_quote(match_id, typ=None, linie: float = None, richtung: str = None, nur_live: bool = True):
    ...
    v59.18 FIX: linie/richtung ergänzt, damit gezielt die zum Tipp passende Quote geholt
    wird statt (wie vorher) blind die höchste Quote über alle Linien hinweg.
    v59.24 FIX: nur_live=True (Standard) zeigt bei Live-Signalen KEINE veraltete
    Vorspiel-Quote mehr an. Nur für echte Pre-Match-Bots explizit nur_live=False setzen.
    """
    return af_odds_beste_quote(match_id, linie=linie, richtung=richtung, nur_live=nur_live)
...
def ls_get_match_result(match_id: str, home: str = "", away: str = "", liga: str = ""):
    ...
    # v59.19 FIX (KRITISCH): TheSportsDB (Quelle 3 weiter unten) liefert Spielstände auch
    # für noch LAUFENDE Spiele, ohne das zu kennzeichnen. Bisher wurde jedes Ergebnis, das
    # NUR von einer einzigen Quelle kam, blind als "Spiel beendet" (FT) akzeptiert – dadurch
    # wurden Signale schon nach wenigen Minuten fälschlich als "verloren" ausgewertet, obwohl
    # das Spiel gerade erst angepfiffen wurde (z.B. "0:0 nach 2 Minuten" wurde als "HZ1 mit
    # 0 Toren beendet" fehlinterpretiert). Jetzt: Wenn API-Football (unsere direkteste, mit
    # Abstand zuverlässigste Quelle) das Spiel JETZT noch als live listet, wird gar nicht erst
    # versucht ein Ergebnis von einer unsicheren Zusatzquelle zu akzeptieren – das Spiel ist
    # dann per Definition noch nicht vorbei, unabhängig davon was andere Quellen behaupten.
    try:
        live_ids_check = {str(m.get("id")) for m in get_live_matches()}
        if str(match_id) in live_ids_check:
            print(f"  [Triple] Fixture {match_id} läuft laut API-Football noch – kein Ergebnis akzeptiert")
            return None
    except Exception as e:
        print(f"  [Triple] Live-Vorab-Check Fehler: {e}")
...
if __name__ == "__main__":
    ...
    print("  Auf Basis von v58 (API-Football Migration + Lock-Fixes)")
    print("=" * 55 + "\n")
